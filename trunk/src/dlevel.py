...
def create_room(map, room):
    ...
    for x in range(room.x1 + 1, room.x2):
       for y in range(room.y1 + 1, room.y2):
            map[x][y] = Cell("cmap, floor of a room")

    # Add wall tiles surrounding the room
    for x in range(room.x1 + 1, room.x2):
        map[x][room.y1] = Cell("cmap, wall, horizontal")
    for x in range(room.x1 + 1, room.x2):
        map[x][room.y2] = Cell("cmap, wall, horizontal")
    for y in range(room.y1 + 1, room.y2):
        map[room.x1][y] = Cell("cmap, wall, vertical")
    for y in range(room.y1 + 1, room.y2):
        map[room.x2][y] = Cell("cmap, wall, vertical")

    # Add corner tiles
    map[room.x1][room.y1] = Cell("cmap, wall, top left corner")
    map[room.x2][room.y1] = Cell("cmap, wall, top right corner")
    map[room.x1][room.y2] = Cell("cmap, wall, bottom left corner")
    map[room.x2][room.y2] = Cell("cmap, wall, bottom right corner")
    ...
    while visited_cells < total_cells:
        ...
        if possible_moves.count(1):
            ...
            if move_direction == DIR_LEFT:
                map[cur_cell[0] - 1][cur_cell[1]] = Cell("cmap, floor of a room")
                cur_cell[0] -= 2
            elif move_direction == DIR_RIGHT:
                map[cur_cell[0] + 1][cur_cell[1]] = Cell("cmap, floor of a room")
                cur_cell[0] += 2
            elif move_direction == DIR_UP:
                map[cur_cell[0]][cur_cell[1] - 1] = Cell("cmap, floor of a room")
                cur_cell[1] -= 2
            ...
            if map[x][y].cell_class != 'wall':
                ...
                if connections == 1 and random.random() < braid_degree :
                    if connection == DIR_LEFT and x < w - 1:
                        map[x + 1][y] = Cell("cmap, floor of a room")
                    if connection == DIR_RIGHT and x > 0:
                        map[x - 1][y] = Cell("cmap, floor of a room")
                    if connection == DIR_UP and y < h - 1:
                        map[x][y + 1] = Cell("cmap, floor of a room")
